...
if (channelCode === undefined || productCode === undefined) {
  console.error(
    "\n  Usage: pnpm --filter @slk/sync publish-one <channelCode> <productCode>\n",
  );
  process.exit(1);
}
...
if (row === undefined) {
  console.error(`\n  No consignment ${productCode}, or no channel "${channelCode}".\n`);
  process.exit(1);
}

if (!row.is_serialised) {
  console.error(
    `\n  ${productCode} is a pooled product type — channel_batch_sellable has no ` +
      `honest number for it. Only serialised designs can be listed per batch today.\n`,
  );
  process.exit(1);
}

if (row.retail_minor === null) {
  console.error(`\n  ${productCode} has no retail price, on the batch or the line. Set one first.\n`);
  process.exit(1);
}
...
if (locations.nodes.length === 0) {
  console.error("\n  This Shopify store has no locations at all — nothing to set inventory against.\n");
  process.exit(1);
}
...
if (result.productSet.userErrors.length > 0) {
  console.error("\n  Shopify refused it:");
  for (const e of result.productSet.userErrors) console.error(`    ${e.field.join(".")}: ${e.message}`);
  process.exit(1);
}
...
if (variant === undefined) {
  console.error(`\n  Shopify accepted the product but returned no variant — nothing to link. Product: ${product.id}\n`);
  process.exit(1);
}
