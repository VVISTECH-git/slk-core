...
export const clothItem = pgTable(
  "cloth_item",
  {
     ...
     * a record made from this cloth already carries both.
     */
...
export const thaan = pgTable(
  "thaan",
  {
    ...
    /**
     * The Product Management record (a colourway: one design, one colour)
     * this Thaan was sorted into when it came back from Print — chosen at
     * the door by colour and motif, which is all Print decides; everything
     * else the record needs came from the bale's cloth item. Null before
     * Print, and for a Thaan received "just as Thaans" that nobody has
     * sorted yet. Follows the Thaan through every later stage unless
     * someone moves it.
     */
    colourwayId: uuid("colourway_id").references(() => colourway.id, { onDelete: "restrict" }),

    /**
     * The shelf piece this Thaan became once its record went on the shelf
     * — the same physical cloth, now stock, and the piece's code is this
     * Thaan's own code so the label stitched on at the start is the one
     * scanned at the till. Null until then; set once and never moved.
     */
  ...
  (t) => [
    ...
    index("thaan_colourway_id_idx").on(t.colourwayId),
