...
function cellText(row: BaleRow, key: ColumnKey): string {
  switch (key) {
    ...
    case "invoiceAmount":
      return row.invoiceAmount === null ? "" : String(row.invoiceAmount);
    case "status":
...
function draftFrom(row: BaleRow): BaleDraft {
  return {
    ...
    invoiceAmount: row.invoiceAmount === null ? "" : String(row.invoiceAmount),
    type: row.type,
                      ...
                      {columns.map((c) => {
                        ...
                        if (c.key === "status") {
                          ...
                          return (
                            <Cell
                              key={c.key}
                              title={who === null ? STATUS_LABEL[r.status] : `${STATUS_LABEL[r.status]} by ${who}`}
                            >
                              <span
                        ...
                        if (c.key === "thaans") {
                          const qrLabel = r.qrGeneratedCount >= r.thaanCount ? "QR ready" : "QR pending";
                          const qrTitle =
                            r.qrGeneratedCount >= r.thaanCount && r.qrGeneratedByName !== null
                              ? `${qrLabel} — generated by ${r.qrGeneratedByName}`
                              : qrLabel;
                          return (
                            <Cell key={c.key} title={r.thaanCount === 0 ? "—" : qrTitle}>
                              {r.thaanCount === 0 ? (
                                "—"
                              ) : (
                                <>
                        ...
                        if (c.key === "invoiceAmount") {
                          return (
                            <Cell key={c.key} numeric title={r.invoiceAmount === null ? "" : `₹${r.invoiceAmount}`}>
