...
export function VendorLedger({ rows }: { rows: LedgerEntryRow[] }) {
  ...
  const payTotal = selectedRows.reduce((sum, r) => sum + r.amount, 0);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      <Header
        title="Vendor Ledger"
        lede="Every stage billed and every payment made, across every vendor. Approve a transaction, then select the approved, unpaid ones for a vendor to settle them in one payment."
      />
    ...
    <Drawer open title={`${entry.vendorName} — ${entry.stage} — ${entry.date}`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <p className="text-[12.5px] text-muted">
          {entry.pieceCount} piece{entry.pieceCount === 1 ? "" : "s"}, ₹{entry.amount.toLocaleString("en-IN")}
          ...
          <ul className="max-h-40 divide-y divide-rule overflow-y-auto rounded-lg border border-rule text-[12.5px]">
            {entries.map((e) => (
              <li key={e.id} className="flex items-center justify-between px-3 py-1.5">
                ...
                <span className="font-mono tabular-nums text-ink-2">₹{e.amount.toLocaleString("en-IN")}</span>
              </li>
            ))}
          </ul>
        </div>
